...
def _verify_requirements(requirements: str, final_components: list[dict], client,
    ...
    prompt = (
        ...
        "one is NOT sufficient. In contrast, some components are "
        "CONVENTIONALLY sourced as complete pre-assembled modules connected "
        "via a header precisely because the bare part isn't practical to "
        "hand-solder directly (e.g. small OLED/LCD glass displays are almost "
        "always bought as an assembled module with a 4-pin header, since the "
        "glass panel itself typically can't survive reflow soldering) -- for "
        "these, a header connector to an external module IS the objectively "
        "correct, standard choice, not a shortcut, and should PASS. Check not just that "
