...
def _extract_requirements(user_request: str, client) -> str:
    ...
    prompt = (
        ...
        "requirement. IMPORTANT: if a requirement involves MULTIPLE distinct "
        "components (e.g. 'a sensor and a display'), list them as SEPARATE "
        "bullet points, one component per bullet -- do not combine multiple "
        "components into one bullet, since that makes it too easy to verify "
        "only one of them and let it excuse the other. Keep it to 2-6 bullet "
        "points, no more."
    )
...
def _verify_requirements(requirements: str, final_components: list[dict], client,
    ...
    prompt = (
        ...
        "each must stand on its own. IMPORTANT: whether a generic connector/"
        "pin-header satisfies a component requirement depends on REAL "
        "engineering practice for that specific part category, not a blanket "
        "rule -- use your own knowledge: sensor/regulator/MCU ICs (e.g. a "
        "temperature sensor, an LDO) are normally available and used as bare, "
        "hand-solderable components, so a generic connector standing in for "
        "one is NOT sufficient -- this applies EVEN IF a different component "
        "in the SAME requirement (e.g. a display) legitimately uses a "
        "connector; check each named component in the requirement text "
        "SEPARATELY, do not let one satisfied component excuse another. In "
        "contrast, some components are CONVENTIONALLY sourced as complete "
        "pre-assembled modules connected via a header precisely because the "
        "bare part isn't practical to hand-solder directly (e.g. small "
        "OLED/LCD glass displays are almost always bought as an assembled "
        "module with a 4-pin header, since the glass panel itself typically "
        "can't survive reflow soldering) -- for these specifically, a header "
        "connector to an external module IS the objectively correct, "
        "standard choice, not a shortcut, and should PASS. CONCRETE EXAMPLE: "
        "if a requirement says 'I2C sensor and I2C display', and the board "
        "has TWO generic 4-pin headers both just wired to the I2C bus with "
        "no real sensor IC anywhere in the BOM, that is a FAIL (only the "
        "display connector is legitimate; the sensor requirement is NOT met "
        "since no real sensor IC exists on the board -- a header alone does "
        "not count as 'having' the sensor). Check not just that "
